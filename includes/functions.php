...
function food_invoice_render_public($id) {
    ...
    <html><head>
        ...
        <style>
            ...
            @media print {
                ...
                .invoice-header { margin-top: 36px !important; }
                .invoice-header-row { margin-top: 12px !important; }
            }
        </style>
    </head><body<?= $auto_print ? ' onload="window.print()"' : '' ?>>
    <div class="wrap" id="invoice-print-area">
        ...
        <table class="invoice-meta">
            <tr><td><strong>Name:</strong> <?= esc_html($client) ?></td><td><strong>Date:</strong> <?= date('d/m/Y', strtotime($date)) ?></td></tr>
            <tr><td><strong>Function:</strong> <?= esc_html($function) ?></td><td><strong>Venue:</strong> <?= esc_html($venue) ?></td></tr>
        </table>
        <table class="invoice-table">
            <thead>
                <tr>
                    <th>S.No.</th><th>Description</th><th>Quantity</th><th>Rate (₹)</th><?php if ($show_discount): ?><th>Discount (₹/qty)</th><?php endif; ?><th>Amount (₹)</th>
                </tr>
            </thead>
            <tbody>
        ...
        <table class="totals-table">
            <tr><td style="text-align:right;"><strong>Total Amount:</strong></td><td style="width:150px;text-align:right;">₹ <?= number_format($total,2) ?></td></tr>
            <?php if ($show_advanced): ?><tr><td style="text-align:right;"><strong>Advanced Payment:</strong></td><td style="width:150px;text-align:right;">₹ <?= number_format($adv,2) ?></td></tr><?php endif; ?>
            <tr><td style="text-align:right;"><strong>Final Amount Due:</strong></td><td style="width:150px;text-align:right;"><strong>₹ <?= number_format($total-floatval($adv),2) ?></strong></td></tr>
        </table>
    ...
    .invoice-print-btn:hover, .invoice-print-btn:focus {
        background: #1742a0;
        box-shadow: 0 4px 16px rgba(37,99,235,0.15);
    }
    .invoice-title { text-align: center; font-size: 1.7em; font-weight: bold; margin-bottom: 0.5px; letter-spacing: 1px; font-family: Arial, Helvetica, sans-serif; }
    .invoice-header { text-align: center; font-weight: bold; font-size: 2.3em; margin-bottom: 0.4em; margin-top: 36px; }
    .invoice-subheader, .invoice-contact, .invoice-address { text-align: center; font-size: 1.18em; margin-bottom: 8px; }
    .invoice-subheader { font-size: 1.25em; text-decoration: underline; text-underline-offset: 4px; font-weight: 600; }
    </style>
    </body></html>
    <?php
} 